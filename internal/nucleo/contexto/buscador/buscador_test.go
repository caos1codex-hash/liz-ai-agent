...
func TestRRF_FusionDosRankings(t *testing.T) {
        b := NuevoBuscador()
        b.Indexar(FragmentoBuscable{ID: "f1", Contenido: "auth"})
        b.Indexar(FragmentoBuscable{ID: "f2", Contenido: "auth"})
        b.Indexar(FragmentoBuscable{ID: "f3", Contenido: "auth"})

        // Vector ranking dice que f2 es el mejor
        ...
        // f2 debería estar en top (rankeado bien en ambos)
        if resultados[0].Fragmento.ID != "f2" {
                t.Errorf("top debería ser f2 (beneficiado por RRF), got %s", resultados[0].Fragmento.ID)
        }
}
