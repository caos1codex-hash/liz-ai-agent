...
func (be *BuscadorEmbeddings) BuscarVector(query string, topK int) ([]ResultadoBusqueda, error) {
	...
	for i := 0; i < topK; i++ {
		r := resultados[i]
		frag := be.fragmentos[r.id] // heredado de Buscador (sin lock, estamos en RLock)
...
// sqrt calcula raíz cuadrada sin importar math (mantén imports mínimos en este archivo).
// Implementación: método de Newton, 5 iteraciones (suficiente para normalización).
