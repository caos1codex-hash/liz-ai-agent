...
// TestHechos_ConcurrentAgregar lanza 20 goroutines agregando hechos para el
// mismo usuario simultáneamente para verificar seguridad concurrente del gestor.
func TestHechos_ConcurrentAgregar(t *testing.T) {
        g, _ := nuevoGestorTest(t)

        var wg sync.WaitGroup
        errs := make(chan error, 20)

        // 20 goroutines agregando hechos para el mismo usuario
        for i := 0; i < 20; i++ {
                wg.Add(1)
                go func(id int) {
                        defer wg.Done()
                        sujeto := fmt.Sprintf("recurso_%d", id)
                        predicado := fmt.Sprintf("estado_%d", id)
                        objeto := fmt.Sprintf("valor_%d", id)
                        _, err := g.AgregarHecho("user-1", sujeto, predicado, objeto, 0.8, "sesion-concurrent")
                        if err != nil {
        ...
        // Verificar que todos los hechos fueron persistidos
        hechos, err := g.Hechos().HechosActivos("user-1")
        if err != nil {
                t.Fatalf("error obteniendo hechos: %v", err)
        }
        if len(hechos) != 20 {
                t.Errorf("debería tener 20 hechos activos, got %d", len(hechos))
        }
}

// TestSesiones_ConcurrentAgregarMensaje lanza 10 goroutines agregando mensajes
// simultáneamente a la sesión activa de un usuario.
func TestSesiones_ConcurrentAgregarMensaje(t *testing.T) {
        g, _ := nuevoGestorTest(t)

        // Crear sesión activa
        _, err := g.NuevaSesion("user-1", "proyecto-concurrent")
        if err != nil {
                t.Fatalf("error creando sesión: %v", err)
        }
        ...
        resultados := make(chan *Mensaje, 10)

        // 10 goroutines agregando mensajes
        for i := 0; i < 10; i++ {
                wg.Add(1)
                go func(id int) {
                        defer wg.Done()
                        contenido := fmt.Sprintf("Mensaje concurrente número %d del usuario", id)
                        msg, err := g.AgregarMensaje("user-1", RolUsuario, contenido)
                        if err != nil {
                                errs <- fmt.Errorf("goroutine %d: %v", id, err)
                                return
                        }
        ...
        // Verificar que se recibieron 10 mensajes
        count := 0
        for range resultados {
                count++
        }
        if count != 10 {
                t.Errorf("debería tener 10 mensajes, got %d", count)
        }

        // Verificar que la sesión tiene 10 mensajes
        activa := g.Sesiones().SesionActiva("user-1")
        if activa == nil {
                t.Fatal("sesión activa no debería ser nil")
        }
        if len(activa.Mensajes) != 10 {
                t.Errorf("la sesión debería tener 10 mensajes, got %d", len(activa.Mensajes))
        }
}
...
// TestHechos_ConflictosMultiples agrega 3 hechos con el mismo sujeto/predicado
// pero diferentes objetos. Solo el último debería quedar activo, los anteriores
// deberían estar marcados como obsoletos.
func TestHechos_ConflictosMultiples(t *testing.T) {
        g, _ := nuevoGestorTest(t)

        sujeto := "usuario"
        predicado := "prefiere_lenguaje"

        h1, _ := g.AgregarHecho("user-1", sujeto, predicado, "Python", 0.7, "sesion-1")
        h2, _ := g.AgregarHecho("user-1", sujeto, predicado, "Rust", 0.8, "sesion-2")
        h3, _ := g.AgregarHecho("user-1", sujeto, predicado, "Go", 0.95, "sesion-3")

        // Solo debería haber 1 hecho activo (el último: Go)
        hechos, _ := g.Hechos().HechosActivos("user-1")
        if len(hechos) != 1 {
                t.Errorf("debería tener 1 hecho activo, got %d", len(hechos))
        }
        if hechos[0].Objeto != "Go" {
                t.Errorf("el hecho activo debería ser 'Go', got %s", hechos[0].Objeto)
        }
        if hechos[0].ID != h3.ID {
                t.Error("el hecho activo debería ser h3 (el último agregado)")
        }

        // Debería haber 2 hechos obsoletos (Python y Rust)
        stats, _ := g.Hechos().Estadisticas("user-1")
        if stats.HechosObsoletos != 2 {
                t.Errorf("debería tener 2 hechos obsoletos, got %d", stats.HechosObsoletos)
        }
        if stats.TotalHechos != 3 {
                t.Errorf("debería tener 3 hechos totales, got %d", stats.TotalHechos)
        }

        // Verificar IDs de los hechos creados
        if h1.ID == h2.ID || h2.ID == h3.ID || h1.ID == h3.ID {
                t.Error("los IDs de hechos conflictivos deberían ser diferentes")
        }
}
